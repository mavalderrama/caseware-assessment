...
SAMPLE_CASES_2 = [
    ...
    {"customer_idx": 0, "title": "Report scheduler skips weekends",          "description": "Scheduled PDF reports set to run daily are not generated on Saturdays or Sundays despite the cron expression being '0 8 * * *'.",   "status": "open"},
    {"customer_idx": 0, "title": "Audit log entries missing user ID",        "description": "Audit trail events fired from the background worker omit the actor user_id field, making compliance review impossible.",             "status": "open"},
    {"customer_idx": 0, "title": "Dark mode breaks data table contrast",     "description": "In dark mode, table row text renders as #1a1a1a on a #1e1e1e background. Affects all pages with DataGrid components.",              "status": "open"},
    # Cyberdyne Corp (index 1)
    {"customer_idx": 1, "title": "Multi-region failover not triggering",     "description": "Health check endpoint returns 200 from the primary region even during a simulated outage. Failover to eu-west-1 never activates.",    "status": "open"},
    {"customer_idx": 1, "title": "Object storage presigned URLs expire early","description": "Presigned S3 URLs sent via email expire after 5 minutes instead of the configured 24 hours. Bucket policy looks correct.",           "status": "open"},
    {"customer_idx": 1, "title": "GraphQL subscriptions drop after 60s",     "description": "WebSocket connections for GraphQL subscriptions are silently terminated after exactly 60 seconds. The ALB idle timeout is 120s.",      "status": "resolved"},
    # Soylent Systems (index 2)
    {"customer_idx": 2, "title": "Pagination breaks on filtered result sets","description": "Applying a status filter and navigating to page 3 returns the same rows as page 1. Offset calculation ignores active filters.",       "status": "open"},
    {"customer_idx": 2, "title": "PDF invoice font renders as boxes on Linux","description": "Invoices generated on the server display correctly on macOS but show empty boxes instead of characters when opened on Linux.",        "status": "open"},
    {"customer_idx": 2, "title": "Session token not invalidated on logout",  "description": "After clicking logout the JWT remains valid for its full TTL. Re-submitting the token to /api/me returns 200 instead of 401.",        "status": "open"},
]
